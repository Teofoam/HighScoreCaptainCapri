...
| Workflow | 参数 | 默认 |
| --- | --- | --- |
| Daily Problem Pusher | `daily_count` / `generated_count` | 10 / 6 |
...
**数制与码制**（数电）：进制转换直接写目标进制的结果，前导零、`0x` / `0b` 前缀、
`H` / `B` / `O` 后缀、下标写法 `(1011)₂` 和分位空格都认；但二进制里写出个 `2` 会判错
—— 那是真错了，不是写法问题。位串（BCD、格雷码、原/反/补码）连着写或四位一组用
空格分开都行。小数转换按"截断到指定位数"比对，少写的尾零自动补齐，所以 `0.1011`
和 `0.10110000` 都算对，但截少一位就不行。

定点小数的机器数按教材写法 `1.01011`（符号位 + 小数点 + 数值位），**位数不补齐**
到 8 位 —— 小数机器数的位数就是原数的位数，补零会把 `-0.1100` 和 `-0.11000000`
变成两道答案不同的题。补码运算题只出不溢出的：溢出时"结果真值"那一空根本没有
正确答案可填。
...
课程用的是《数字逻辑设计基础》（第二版），理论 40 学时，分配如下 ——
**生成器的优先级该照这个排，而不是照章节顺序**：

| 章节 | 学时 | 覆盖 |
| --- | --- | --- |
| 08 时序逻辑电路 | 9 | ❌ |
| 03 逻辑代数基础 | 8 | ✅ `sop-simplify` |
| 06 组合逻辑电路 | 8 | ❌ |
| 04 VHDL 硬件描述语言 | 4 | ❌ |
| 07 集成触发器 | 4 | ❌ |
| 02 数制与码制 | 3 | ✅ `number-system` |
| 12 可编程逻辑器件 | 2 | ❌ |
| 01 概述 / 10 半导体存储器 | 各 1 | — |

- [x] `number-system`：数制转换（含小数、指定保留位数）、精度截位、
      8421 / 余 3 BCD、格雷码、原反补码（含定点小数）、补码运算
...
对着教材目录数过一遍：**模电的大半既不需要 SPICE，也不需要画图**。
这些题的标准答案就是教材的估算公式本身，纯 Python 算得出来，判题器
（`Numeric` + 误差传递）也是现成的。所以先把它们做掉、让模电通道尽早
跑起来，再回头啃真正需要基建的部分。

先做（不需要新基建，一周一个）：

- [ ] `zener`：稳压管电路求 $U_o$ / $I_Z$，判断是否进入稳压状态（§2.2.1）
- [ ] `opamp-calc`：反相 / 同相 / 加减 / 积分运算电路求 $u_o$（§4.1.1）
- [ ] `comparator`：迟滞比较器的阈值 $U_{T+}$ / $U_{T-}$（§4.1.3）
- [ ] `rectifier`：整流滤波，$U_{o(AV)}$、二极管反峰电压、$C$ 取值（§7.2）
- [ ] `power-amp`：OCL / OTL 的 $P_{om}$、$\eta$、管耗（§6.2）
- [ ] `freq-response`：$f_H$ / $f_L$、$-3\mathrm{dB}$、波特图读数（§4.2.1）——
      `quantity.py` 的 dB 绝对容差正好用在这
- [ ] `ce-amp`：共射放大电路 Q 点 / $A_v$ / $R_i$ / $R_o$，多空 + 误差传递
      （§2.4.3–2.4.4）。**用教材的解析公式出答案，不套 SPICE** ——
      教材的估算法本身就是闭式解，仿真在这一步是多余的

后做（这些才真的需要新基建）：

- [ ] `analog/topology.py`：电路拓扑作为唯一真相源（图、网表、答案同源）
- [ ] `analog/schematic.py`：schemdraw 出电路图，标注与参数同源
- [ ] `sim/ngspice.py`：走 `subprocess` 调 `ngspice -b` 解析输出
      （**不用 PySpice**，它绑 libngspice 共享库，CI 里折腾成本高）
- [ ] `feedback`：反馈组态判断（电压/电流 × 串联/并联），必须有电路图（§3.4）
- [ ] `analog/faults.py`：故障变异 → 改错题（问故障在哪，或问故障后的 Q 点）
- [ ] **spec 验证判题**：给指标让你设计，把你的参数灌进 SPICE 验达标 —— 
      任何合法解都算对，这是唯一能自动判"设计题"的路子。SPICE 真正不可
      替代的就是这一条，它是终点不是起点
- [ ] 波形识别：扰动电路生成干扰项，把"画波形"转成四选一

> 教材章节权重（按页数，用来定生成器的 weight）：第二章 90 页正文 + 10 页习题，
> 是第三、四章的两倍，全书最重；其后是第三章（53 页）、第四章（46 页），
> 第七、五、六章各 30～40 页，第八章偏阅读、习题最少。
...
先说定位：Capri 是**应试导向**的。题目和判题一律按课程教材的约定走
（估算法、$U_{BE}$ 取 0.7、$r_{be}$ 用教材那个公式），不是按器件物理的
严格模型。真想搞懂模电得去读 Razavi、Pierret 那类书 —— 那是另一件事，
这个 bot 不承担也承担不了。它的目标很窄：把该刷的题刷够，把该记的结论记牢。

- **判过程分**。这个 bot 的形状是"验证结果"，不是"批改推导"。
  多步推导按步给分、实验报告 —— 这些不在射程内，硬改就是重写另一个项目。
  阶段 3 的 spec 验证是绕过这个限制的办法：不判过程，直接验产物。

- **Multisim 仿真作业**。模电实验课点名要 Multisim，但它是 GUI 原理图工具，
  bot 伸不进去，作业还得自己点。唯一的交集是：阶段 3 的 ngspice 和 Multisim
  底层都是 SPICE，真做到那步，两边的仿真结果可以互相对账。
