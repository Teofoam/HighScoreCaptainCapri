...
`H` / `B` 后缀、下标写法 `(1011)₂` 和分位空格都认；但二进制里写出个 `2` 会判错 ——
那是真错了，不是写法问题。位串（BCD、格雷码、原/反/补码）连着写或四位一组用空格
分开都行。小数转换按"截断到指定位数"比对，少写的尾零自动补齐，所以 `0.1011` 和
`0.10110000` 都算对，但截少一位就不行。
...
- [x] `number-system`：数制转换、精度截位、BCD / 格雷码 / 原反补码（五种变体）
